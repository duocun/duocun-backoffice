...
const AddressSearch = ({label, placeholder, handleSelectLocation, location}) => {
  ...
  const handleSelectData = (address) => {
    ...
    ApiLocationService.getLocationByGeocode({placeId}).then(({data}) => {
      const location = (!data.data.addressNumber) ? {...data.data, streetNumber} : data.data;
...
{/* </Throttle> */}
    ...
    /> */}
        {
          addresses && addresses.length > 0 && searching &&
          addresses.map(d => 
            <MenuItem className={classes.listItem} key={d.placeId} value={d.placeId} onClick={() => handleSelectData(d)}>
              <div>{d.mainText}{ d.secondaryText? `, ${d.secondaryText}`: ''}</div>
            </MenuItem>
          )
        }
    </div>
