...
const AccountFormPage = ({ match, account, setAccount }) => {
      ...
      <React.Fragment>
        ...
        <GridItem xs={12} md={6} lg={6}>
          <TextField
            ...
            onChange={e => {
              setModel({ ...model, imageurl: e.target.value });
            }}
          />
        </GridItem>
        <GridItem xs={12} md={6} lg={6}>
          <TextField
            ...
            value={model.realm}
            onChange={e => {
              setModel({ ...model, realm: e.target.value });
            }}
          />
        </GridItem>
        <GridItem xs={12} md={6} lg={6}>
          <TextField
            ...
            value={model.sex}
            onChange={e => {
              setModel({ ...model, sex: e.target.value });
            }}
          />
        </GridItem>
        <GridItem xs={12} md={6} lg={6}>
                ...
                return (
                  <GridItem xs={6} lg={6}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={model.attributes.indexOf(_key) >= 0}
                          onChange={e => _attributeClick(e, _key)}
                          color="primary"
                        />
                      }
  ...
  const renderRight = () => {
    return <GridContainer>{_renderAttributes()}</GridContainer>;
  };
  const renderLeft = () => {
  ...
  const saveModel = () => {
    ...
    if (model && (!model._id || model._id === "new")) {
      ApiAccountService.createAccount(model).then(({ data }) => {
        setProcessing(false);
      ...
      ApiAccountService.updateAccount(model._id, model).then(({ data }) => {
        setProcessing(false);
        if (data.code === "success") {
          // success
          setAlert({
            ...
            <GridContainer>
              <GridItem xs={12}>
                <Button
                  ...
                  variant="contained"
                  disabled={
                    loading || processing || !model.username || !model.type
                  }
                  onClick={saveModel}
                >
                  <SaveIcon /> {t("Save")}
